...
<center>
  <img class="pic" src="../picture/note8.jpg"  />  
 </br>
 <?php
if ($db=lacz()){

	$zapytanie = "SELECT nazwa,cena,specyfikacja FROM modele where id=2";
...
<center>
<font color="pink" size="6">Podziel się swoją opinią! </font>
...
<div class="stars">
	...
	Ocena:
	<textarea placeholder="Miejsce na ocene"  name="area" maxlength="100">  </textarea>
	<select name="model">
<?php 

if ($db=lacz()){
	$model=$rekord->$model;
$zapytanie = "Select nazwa from modele";
$db->query($zapytanie);
$jedno = $db->query($zapytanie);
echo"<option> Wybierz model </option>";
echo"<option>  Samsung Note8 </option>";
echo"<option>   Samsung S9      </option>";
echo"<option>   Samsung S8      </option>";
echo"<option>   Samsung S7     </option>";
echo"<option>   Samsung A8      </option>";
echo"<option>   Samsung S6        </option>";
echo"<option>   Samsung J5       </option>";
echo"<option>  Xcover4       </option>";
echo"<option>   Samsung S5    </option>";

//while($drugie = $jedno->fetch_object()){

//echo"<option value=\"$jedno->nazwa\">$drugie->nazwa</option>";
}
//}
else 
{
...
<div class="sec">
<input type="text" placeholder="Pseudonim" name="pseudo" maxlength="15" />
...
echo "Baza danych pomyślenie załączona."	;
	$zapytanie = "SELECT stars,opis,pseudo,model FROM ocena  ";
	$wynik = $db->query($zapytanie);
	
	
	 
	if ($n=$wynik->num_rows) {
		echo "</br><b> Ilość opinii: $n </b> </br> ";
	 echo "<table >";
	 echo "<tr >  <td align='center'><b> Gwizadki </td> <td align='center'><b> Opis </td>
	<td align='center'><b>Pseudo </td> <td align='center'><b> Model </td> </tr>   ";
